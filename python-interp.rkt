...
(define-primf (Range (n VNum?) & ret)
  (if (and (and (exact-integer? (VNum-n n)) (andmap VNum? ret)) (andmap (lambda (x) (exact-integer? (VNum-n x))) ret))
      (cond
        [(empty? ret) (if (>= 0 (VNum-n n))
                          (m-return (VList empty))
                          (m-return (VList (map (lambda (x) (VNum x)) (range 0 (VNum-n n) 1)))))]
        [(= 1 (length ret)) (m-return (VList (map (lambda (x) (VNum x)) (range (VNum-n n) (VNum-n (first ret)) 1))))]
        [(= 2 (length ret)) 
         (if (= 0 (VNum-n (second ret)))
             (interp-error "TypeError")
             (m-return (VList (map (lambda (x) (VNum x)) (range (VNum-n n) (VNum-n (first ret)) (VNum-n (second ret)))))))]
        [else (interp-error "TypeError")])
      (interp-error "TypeError")
      ))
